...
from textimagecloud.textimage_cloud import TextImageCloud
...
class GenerateCLIArguments(CLIBaseArguments):
    ...
    def parse(arguments: list[str]):
        parser = argparse.ArgumentParser(
            ...
            description='''
            Generate an \'TextImageCloud\' from a csv file indicating image filepath, text data, and weight for image.
            '''
        )
...
def generate(args: GenerateCLIArguments | None = None) -> None:
    ...
    weighted_textimages: list[WeightedTextImage] = load_weighted_text_images(args.input)
    total_textimages = len(weighted_textimages)
    args.logger.info('loaded {0} weights, texts, images'.format(total_textimages))

    textimage_cloud = TextImageCloud(
        logger=args.logger,
        mask=args.mask,
    ...
    args.logger.info('generating textimagecloud from {0} weighted and normalized texts, images.{1}'.format(
        total_textimages,
    ...
    layout = textimage_cloud.generate(weighted_textimages, cloud_expansion_step_size=args.cloud_expansion_step_size)
    if args.maximize_empty_space:
        args.logger.info('Maximizing {0} text images: expanding them to fit their surrounding empty space.'.format(len(layout.items)))
        layout = textimage_cloud.maximize_empty_space(layout)
